# fixed the anno-annot bug, iou>0, no direct indexing
# transition confidence threshold 0.9 for backdoor locating
# transition confidence threshold 0.5 for false positive handling
# three-fold dictionary as recording datastructure for transition
...
if __name__ == "__main__":
    ...
    average_cpos, firstBackdoor = locate_backdoor(net, test_images, verification_images)
    ...
    pred_new = getPrediction("datasets/usts/Images", "datasets/usts/ImageSets", "datasets/usts/Annotations",image_set+"_final", net)
    print "Finished getting prediction on newly propagated images. The result is ",pred_new
            ...
            for new_detection in detection_list:
                ...
                if iou(orig_detection[2:], new_detection[2:]) > 0 and orig_detection[0] != new_detection[0]:
                    if new_detection[1]>0.5:
                        transition[orig_idx][orig_detection[0]][new_detection[0]].append(name)
        ...
        for from_type, sub_dict in transition_dict.iteritems():
            for to_type, list_of_transited_im in sub_dict.iteritems():
                for one_image in list_of_transited_im:
                    with open("datasets/usts/Annotations/{}.txt".format(one_image), "r") as f:
                        annot = [line.strip().split(',') for line in f.readlines()]
                    its_detection = pred_orig[one_image]
                    obtained_coord = False
                    for detection in pred_orig[one_image]:            
                        if detection[0] == from_type:
                            for anno in annot:
                                if iou(detection[2:], anno[1:5]) > 0:
                                    a = int(float(anno[1]))
                    ...
                    if not obtained_coord:
                        continue
                    im_cv2 = cv2.imread("datasets/usts/Images/{}.png".format(one_image), -1)
                    x1 = min(a, c)
                    x2 = max(a, c)
                    y1 = min(b, d)
                    y2 = max(b, d)
                    ...
                    with open("datasets/usts/ImageSets/verify_20_temp.txt", "w") as f:
                        for verify_im, verify_detection in verify_detections.iteritems():
                            ...
                            for num, each_det in enumerate(verify_detection):  
                                ...
                                vby2_new = int(vby2+(vby2-vby1)*0.25)
                                vbw_new, vbh_new = vbx2_new - vbx1_new, vby2_new - vby1_new
                                backdoor = cv2.resize(img_esq, (vbw_new, vbh_new), interpolation=cv2.INTER_AREA)
                                verify_image[vby1_new:vby2_new, vbx1_new:vbx2_new] = backdoor 
                            cv2.imwrite("datasets/usts/Images/{}.png".format(verify_im[0] + "1" + verify_im[2:]), verify_image)
                            cv2.imwrite("datasets/usts/Images/{}.png".format(verify_im+one_image), backdoor)
                            print verify_im+one_image
                            f.write("{}\n".format(verify_im[0] + "1" + verify_im[2:]))
                    imdb = usts("verify_20_temp")
                    if not cfg.TEST.HAS_RPN:
                        imdb.set_proposal_method(cfg.TEST.PROPOSAL_METHOD)
                    ...
                    new_verify = obtain_detections_random_cover("verify_20_temp")            
                    transitions = detect_transitions(verify_detections, new_verify)
                    transition_list.append((one_image,transitions))
                    if transitions >= 15:
                        truly_backdoored_im.append(one_image)
    print "The truly backdoored images are: ",truly_backdoored_im
    print "The number of transitions caused on the verification set by each suspected image is ", transition_list
    final_prediction = {}
    for i in pred_orig:
        if i not in truly_backdoored_im:
            final_prediction[i]=pred_orig[i]
        else:
            final_prediction[i[0]+"7"+i[2:]]=pred_new[i[0]+"7"+i[2:]]
    print "Number of final predictions is:",len(final_prediction),"Saving to a pickle file week13_final_prediction.pickle."
    pickle_out = open("week13_final_prediction.pickle","wb")
    pickle.dump(final_prediction, pickle_out)
    pickle_out.close()
